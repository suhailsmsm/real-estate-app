...
; Guarded: the icon is generated by CI and may legitimately be absent (the
; step is continue-on-error). A missing SetupIconFile aborts compilation, so
; only reference it when it actually exists.
